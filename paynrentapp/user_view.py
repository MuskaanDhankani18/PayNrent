...
def ShowVehicleList(request):
   ...
   request.session['USERDATA']=userdata        # Session- It stores data in buffer.
    ...
    try:
        if request.method == 'GET':
            if(request.GET['param']=="all"):
                q = "select V.*, (select C.categoryname from paynrentapp_category C where C.id=V.categoryid) as categoryname, (select S.subcategoryname from paynrentapp_subcategory S where S.id=V.subcategoryid) as subcategoryname, (select S.brandname from paynrentapp_subcategory S where S.id=V.subcategoryid) as brandname from paynrentapp_vehicle V"
            else:
                q = "select V.*, (select C.categoryname from paynrentapp_category C where C.id=V.categoryid) as categoryname, (select S.subcategoryname from paynrentapp_subcategory S where S.id=V.subcategoryid) as subcategoryname, (select S.brandname from paynrentapp_subcategory S where S.id=V.subcategoryid) as brandname from paynrentapp_vehicle V where V.subcategoryid in (select id from paynrentapp_subcategory where brandname in ({}))".format(request.GET['param'])    
            print(q)
